...
import { deleteBathEvent, updateBathEvent } from "@/lib/repo";
import { formatTime } from "@/lib/format";
import type { BathEvent } from "@/lib/types";
...
export default function BathEventRow({ event }: { event: BathEvent }) {
  ...
  const save = () => {
    updateBathEvent(event.id, { time: withInputTime(event.time, time), comment: comment.trim() || undefined });
    setEditing(false);
  };
  ...
  return (
    <div className="flex items-center gap-3 py-3 border-b border-border last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        {event.comment && <div className="text-xs text-text-muted italic">{event.comment}</div>}
      </div>
      {confirmDelete ? (
        <div className="flex items-center gap-2">
          <button
            onClick={() => deleteBathEvent(event.id)}
